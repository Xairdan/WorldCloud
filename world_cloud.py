...
class WordCloud:
    ...
    def analyze_text(self, filename: str) -> Dict[str, int]:
        """
    ...
    def draw_word_cloud(self, filename: str) -> None:
        ...
        for word in stats:
            ...
            if stats[word] >= MAX_FONT_SIZE:
                self.write_word(word, color, MAX_FONT_SIZE)
            elif MIN_FONT_SIZE <= stats[word] <= MAX_FONT_SIZE:
                self.write_word(word, color, stats[word])
            else:
                pass
